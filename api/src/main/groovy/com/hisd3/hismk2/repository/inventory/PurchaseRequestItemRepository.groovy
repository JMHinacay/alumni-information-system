...
interface PurchaseRequestItemRepository extends JpaRepository<PurchaseRequestItem, UUID> {
	...
	@Query(value = "select DISTINCT pi.purchaseRequest from PurchaseRequestItem pi where pi.purchaseRequest.isApprove = true and pi.purchaseRequest.consignment = :consignment and pi.refPo is null")
	List<PurchaseRequest> getPrItemsNotYetPo(@Param('consignment') Boolean consignment)
