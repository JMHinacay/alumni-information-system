...
import com.hisd3.hismk2.domain.payroll.Schedule
...
@Component
class TimeKeepingCalculatorService {
...
//            return scheduleComputation
    ...
    List<AccumulatedLogSummary> getAccumulatedLogs(UUID id, Instant startDate, Instant endDate) {

        Map<String, Object> employeeSchedule = getAllSchedule(id, startDate, endDate)
...
//        sample
        ...
        while (date.isBefore(endDate)) {
            ...
            List<AccumulatedLog> finalAccumulatedLogs = []
            ...
            firstLog = reduceSchedule(firstLog)
            AccumulatedLog normalAccumulatedLog = new AccumulatedLog()
            normalAccumulatedLog.inTime = normalAccumulatedLog.inTime
            normalAccumulatedLog.outTime = normalAccumulatedLog.outTime
            normalAccumulatedLog.date = normalAccumulatedLog.date
            normalAccumulatedLog.finalLogs = objectMapper.convertValue(firstLog, FinalAccumulatedLogs)
            normalAccumulatedLog.schedule = new Schedule()
            normalAccumulatedLog.schedule.start = firstLog.scheduleStart
            normalAccumulatedLog.schedule.end = firstLog.scheduleEnd
            normalAccumulatedLog.schedule.type = "NORMAL"
            if (employeeSchedule.get(formattedDate)) {
                normalAccumulatedLog.department = departmentRepository.findById(employeeSchedule.get(formattedDate)['department'] as UUID).get()
            }
            finalAccumulatedLogs.add(normalAccumulatedLog)
            totals = objectMapper.convertValue(firstLog, EmployeeAccumulatedAttendanceDto)
            ...
            Integer count = 1;
            schedule.each {
                EmployeeAccumulatedAttendanceDto secondLog = new EmployeeAccumulatedAttendanceDto()
                secondLog = reduceSchedule(secondLog)
                AccumulatedLog overtimeLog = new AccumulatedLog()
                overtimeLog.inTime = overtimeLog.inTime
                overtimeLog.outTime = overtimeLog.outTime
                overtimeLog.date = overtimeLog.date
                overtimeLog.schedule = new Schedule()
                overtimeLog.schedule.start = secondLog.scheduleStart
                overtimeLog.schedule.end = secondLog.scheduleEnd
                if (!it.isOIC) {
                    secondLog = calculateOvertimeAccumulatedLogs(date, it, employeeRestSchedule, holidays, logs)
                    overtimeLog.schedule.type = "OVERTIME_${count}"
                } else {
                    secondLog = calculateOvertimeOICAccumulatedLogs(date, employeeSchedule, employeeRestSchedule, holidays, logs)
                    overtimeLog.schedule.type = "OVERTIME_OIC_${count}"
                }
                overtimeLog.finalLogs = objectMapper.convertValue(secondLog, FinalAccumulatedLogs)
                overtimeLog.department = departmentRepository.findById(it['department'] as UUID).get()
                finalAccumulatedLogs.add(overtimeLog)
                totals = addAccumulatedLogs(totals, secondLog)
                count++
            }


            //===Regular OIC Schedules===
            if (employeeSchedule.get("${formattedDate}_OIC")) {
                EmployeeAccumulatedAttendanceDto oicLog = calculateOICAccumulatedLog(date, employeeSchedule, employeeRestSchedule, holidays, logs)
                oicLog = reduceSchedule(oicLog)
                AccumulatedLog oicLogFinal = new AccumulatedLog()
                oicLogFinal.inTime = oicLogFinal.inTime
                oicLogFinal.outTime = oicLogFinal.outTime
                oicLogFinal.date = oicLogFinal.date
                oicLogFinal.finalLogs = objectMapper.convertValue(oicLog, FinalAccumulatedLogs)
                oicLogFinal.schedule = new Schedule()
                oicLogFinal.schedule.start = oicLog.scheduleStart
                oicLogFinal.schedule.end = oicLog.scheduleEnd
                oicLogFinal.schedule.type = "OIC"
                if (employeeSchedule.get("${formattedDate}_OIC")) {
                    oicLogFinal.department = departmentRepository.findById(employeeSchedule.get("${formattedDate}_OIC")['department'] as UUID).get()
                }
                finalAccumulatedLogs.add(oicLogFinal)
                totals = addAccumulatedLogs(totals, oicLog)
            }


            //===Leaves Schedules===
            if (employeeSchedule.get("${formattedDate}_LEAVE")) {
                EmployeeAccumulatedAttendanceDto leavesLog = calculateLeaveAccumulatedLogs(date, employeeSchedule)
                leavesLog = reduceSchedule(leavesLog)
                AccumulatedLog leavesLogFinal = new AccumulatedLog()
                leavesLogFinal.inTime = leavesLog.inTime
                leavesLogFinal.outTime = leavesLog.outTime
                leavesLogFinal.date = leavesLog.date
                leavesLogFinal.finalLogs = objectMapper.convertValue(leavesLog, FinalAccumulatedLogs)
                leavesLogFinal.schedule = new Schedule()
                leavesLogFinal.schedule.start = leavesLog.scheduleStart
                leavesLogFinal.schedule.end = leavesLog.scheduleEnd
                leavesLogFinal.schedule.type = "LEAVE"
                if (employeeSchedule.get("${formattedDate}_LEAVE")) {
                    leavesLogFinal.department = departmentRepository.findById(employeeSchedule.get("${formattedDate}_LEAVE")['department'] as UUID).get()
                }
                finalAccumulatedLogs.add(leavesLogFinal)
                totals = addAccumulatedLogs(totals, leavesLog)
            }
//            if (!firstLog.inTime && !firstLog.outTime) {
//                totals.inTime = secondLog.inTime
//                totals.outTime = secondLog.outTime
//            }
            totals.inTime = firstLog.inTime
            totals.outTime = firstLog.outTime
            totals.date = date
            AccumulatedLogSummary summary = new AccumulatedLogSummary()
            summary.totals = objectMapper.convertValue(totals, Totals)


            summary.accumulatedLogs = finalAccumulatedLogs
...
//        accumulatedLog.scheduleStart = schedule.getDateTimeStartRaw()
//        accumulatedLog.scheduleEnd = schedule.getDateTimeEndRaw()
...
//                    days = days.findAll { addedHoursRegularHolidays.indexOf(it) < 0 }
    ...
    EmployeeAccumulatedAttendanceDto calculateOvertimeAccumulatedLogs(
            Instant date,
//            Map<String, Object> employeeSchedule,
            EmployeeScheduleDto currentSchedule,
            Map<String, Object> employeeRestDaySchedule,
            Map<String, List<EventCalendar>> holidays,
            Map<String, List<Map<String, Instant>>> employeeLogs
    ) {
...
//====================================UTILITY METHODS====================================\\
     ...
     *
     * @param scheduleStart this could be the start of a schedule.
     * @param scheduleEnd this could be the end of the schedule
     * @param inTime this could be the the time employee has logged-in in the biometrics.
     * @param outTime this could be the the time employee has logged-out in the biometrics
     * @return a tuple of instants, the left is the considered in and right is considered end.
     * @thows Exception   this will throw an exception if nothing will meet the criteria
     */
    Pair<Instant, Instant> consideredTimesExtractor(Instant scheduleStart, Instant scheduleEnd, Instant inTime, Instant outTime) {
        Instant consideredIn, consideredOut
        if ((scheduleStart.isAfter(inTime) || inTime == scheduleStart) && (outTime.isAfter(scheduleEnd) || outTime == scheduleEnd)) {
            consideredIn = scheduleStart
            consideredOut = outTime
        } else if ((scheduleStart.isAfter(inTime) || inTime == scheduleStart) && (outTime.isBefore(scheduleEnd) || outTime == scheduleEnd)) {
            consideredIn = scheduleStart
            consideredOut = outTime
        } else if ((scheduleStart.isBefore(inTime) || scheduleStart == inTime) && (outTime.isBefore(scheduleEnd) || outTime == scheduleEnd)) {
            consideredIn = inTime
            consideredOut = outTime
        } else if ((scheduleStart.isBefore(inTime) || scheduleStart == inTime) && (outTime.isAfter(scheduleEnd) || outTime == scheduleEnd)) {
            consideredIn = inTime
            consideredOut = outTime
        } else throw new Exception("Something went wrong: Failed to determine the considered time")
...
//===========================================DB Operations===========================================\\
    ...
    Map<String, Object> getAllSchedule(UUID id, Instant startDate, Instant endDate) {
        List<Map<String, Object>> scannedEmployeeSchedule = entityManager.createQuery("""
                Select 
                    ...
                    es.department as e_s_department
...
es.assignedDate as e_s_assigned_date,
            ...
            """).setParameter("id", id)
                ...
                .setParameter("endDate", endDate)
                .unwrap(Query.class)
                .setResultTransformer(new EmployeeScheduleDtoTransformer(false))
                .getResultList()
