...
class MessageBuilderResource {
	...
	OrmDto buildMessage(Department emp_department, Case patientCase, List<OrderSlipItem> ordersItems, String device) {
		...
		header.sendingFacility = "ACEMCB"//emp_department.departmentName
		...
		// Change requesting to always Admitting
		//		Employee emp
		//		if(ordersItems[0].orderslip.parentCase.attendingPhysician)
		//		if (ordersItems[0].orderslip.requestingPhysician != null) {
		...
		//			visits.referringPhysicianName = emp.fullName ?: ""
		//		} else {
		//			visits.referringPhysicianId = ordersItems[0].orderslip.requestingPhysicianPrc
...
//					OrderItem orderItem = new OrderItem()
//					orderItem.identifier = content.service.processCode
//					orderItem.serviceName = content.service.serviceName
//					odr.add(orderItem)
//				}
			} else {
		...
		if (listItems.size() > 0) {
			listItems.each {
				...
				try {
					...
					Message msgPayload = new Message()
					msgPayload.from = deptid
					msgPayload.topic = "Result released"
					msgPayload.message = "Please Reload Data"
					socketService.updatesToDepartment(msgPayload)
				}
				catch (Exception e) {
					println(e)
				}
