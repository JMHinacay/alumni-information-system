...
from pms.cases c left join patient_age p on c.patient = p.id where date_part('year', c.time_of_death) = ${year}""")
    ...
    DohDeathsPageDto countDeathV3(@GraphQLArgument(name = 'year') Integer year) {
        ...
        if (result) {
            ...
            def dischargedResult = jdbcTemplate.queryForList("""
                select sum(sub.total_discharges) from (
                select         
                count(c.*) as total_discharges
                from referential.doh_service_types s 
                left join pms.cases c 
                on upper(s.tsdesc) = upper(c.service_type) and date_part('year', discharged_datetime + INTERVAL '8h') = ?
                and (discharge_disposition in ('DAMA/HAMA','TRANSFERRED','ABSCONDED','DISCHARGED')) and c.registry_type in ('IPD')
                ...
                order by s.tscode asc) as sub;""",
                    year)
            if (dischargedResult) {
                BigInteger totalDischarged = dischargedResult[0]['sum'] as BigInteger
