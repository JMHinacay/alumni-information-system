...
class PurchaseRequestItemService {
	...
	List<PurchaseRequest> getPrItemsNotYetPoCS(@GraphQLArgument(name = "consignment") Boolean consignment = false) {
		return purchaseRequestItemRepository.getPrItemsNotYetPo(consignment).sort{it.prNo}.reverse(true)
	}
