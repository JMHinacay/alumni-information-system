...
interface WeeklyUpdatesRepository extends JpaRepository<WeeklyUpdates, UUID> {

	@Query(
			value = '''Select a from WeeklyUpdates a where a.lotTransaction.id = :lotTransactionId'''
	)
	List<WeeklyUpdates> findByLotTransactionId(@Param("lotTransactionId") UUID lotTransactionId)
