...
interface UserActivityRepository extends JpaRepository<UserActivity, UUID> {

	@Query(
			value = '''Select ua from UserActivity ua where ua.lotTransaction.id = :id'''
	)
	List<UserActivity> findByLotTransactionId(@Param("id") UUID id)
